...
class AbworkoutTracker:
    ...
    def get_thresholds_beginner(self):
        """获取初学者模式的阈值设置"""
        thresholds = {
            'POSE_UP_ANGLE': 150.0,      # 上升阶段最大角度
            'POSE_DOWN_ANGLE': 100.0,     # 下降阶段最小角度
        }
        return thresholds
    ...
    def track(self, k, im0, ind, count):
        ...
        # 计算肩-髋、踝-髋和水平的夹角 6 12 16
        angle_shoulder_hip = calculate_angle(k[6].cpu(), k[12].cpu(), k[16].cpu(), reference_direction='vertical')
        
        # 绘制正确计数
        draw_text(
            im0,
            "CORRECT: " + str(self.state_tracker['ABWORKOUT_COUNT']),
        ...
        draw_text(
            im0,
            "ANGLE: " + str(round(angle_shoulder_hip, 2)),
        ...
        # 获取当前状态
        current_state = self._get_state(angle_shoulder_hip)  # 使用肩髋角度作为主要判断依据
        self._update_state_sequence(current_state)

        # 两头起计数逻辑
        if current_state == 's1':
            if len(self.state_tracker['state_seq']) == 3:
                self.state_tracker['ABWORKOUT_COUNT'] += 1
