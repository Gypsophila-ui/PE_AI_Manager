...
namespace SE_PE_AI_Manager.Controllers
{
    ...
    public class HomeworkController : ControllerBase
    {
        ...
        [HttpPost("get_final_submit")]//获取当前作业下每位学生被计入得分的提交编号
        public ActionResult<API_result> Get_final_submit([FromBody] Request_four request)
        {
            string user_id = request.First;//获取参数
            string jwt = request.Second;
            string course_id = request.Third;
            string homework_id = request.Fourth;
            var API_result = new API_result//初始化的返回值
            ...
            if (user_id == null || jwt == null || course_id == null || homework_id == null)//如果参数错误
            {
                API_result.Success = false;
                API_result.Message = "Error parameters";
                API_result.Data = "NULL";
            ...
            Function_result val = SE_PE_AI_Manager.Operation.Homework.Get_final_submit(user_id, jwt, course_id, homework_id, connection);
            if (val.Code < 0)//如果返回结果存在错误
            {
                API_result.Success = false;
                API_result.Message = val.Message;
