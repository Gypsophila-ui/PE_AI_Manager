...
namespace SE_PE_AI_Manager.Controllers
{
    ...
    public class TestController : ControllerBase
    {
        ...
        [HttpPost("student")]//新建学生数据
        public ActionResult<API_result> Student()
        {
            var API_result = new API_result//初始化的返回值
            {
                Success = true,
            ...
            var UploadData= new SE_PE_AI_Manager.Test.Upload_data(); // 创建实例
            Function_result val = UploadData.student(connection); // 调用实例方法
            if (val.Code < 0)//如果返回结果存在错误
            {
                API_result.Success = false;
                API_result.Message = val.Message;
        ...
        [HttpPost("student_course")]//新建学生-课程数据
        public ActionResult<API_result> Student_course()
        {
            var API_result = new API_result//初始化的返回值
            {
                Success = true,
            ...
            var UploadData = new SE_PE_AI_Manager.Test.Upload_data(); // 创建实例
            Function_result val = UploadData.student_course(connection); // 调用实例方法
            if (val.Code < 0)//如果返回结果存在错误
            {
                API_result.Success = false;
                API_result.Message = val.Message;
        ...
        [HttpPost("submit")]//新建提交数据
        public ActionResult<API_result> Submit()
        {
            var API_result = new API_result//初始化的返回值
            {
                Success = true,
            ...
            var UploadData = new SE_PE_AI_Manager.Test.Upload_data(); // 创建实例
            Function_result val = UploadData.submit(connection); // 调用实例方法
            if (val.Code < 0)//如果返回结果存在错误
            {
                API_result.Success = false;
                API_result.Message = val.Message;
