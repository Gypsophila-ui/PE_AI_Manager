...
namespace SE_PE_AI_Manager.Operation
{
    public class Class
    {
        public static Function_result New_class(string teacher_id, string jwt, string course_id, string title, string description,string content_url, OracleConnection connection)//新建教学课程
        {
            ...
            //如果课程的确是当前教师拥有的，那么就查询当前要新增教学任务的课程的最大教学任务编号
            check = "SELECT max(id) FROM class WHERE course_id = '" + course_id + "'";
            count = 0;
            try
            {
                using (OracleCommand command = new OracleCommand(check, connection))
            ...
            {
                return_result.Code = -15;//-15表示查询课程中的最大教学任务编号的sql操作失败
                return_result.Message = "SQL Error";
                return return_result;
            }
            count++;//当前教学任务的ID为当前课程对应的最大教学任务ID+1
            //以下为数据库插入操作
            string action = "INSERT INTO class(id, course_id, title, description, CONTENT_URL, CREATE_TIME)\n";
            action = action + "values('" + Convert.ToString(count) + "',";
            action = action + "'" + course_id + "',";
            action = action + "'" + title + "',";
            action = action + "'" + description + "',";
            action = action + "'" + content_url + "',";
            ...
            {
                return_result.Code = -16;//-16表示插入教学任务的sql操作无法顺利执行
                return_result.Message = "SQL Error";
                return return_result;
            }
        ...
        public static Function_result Edit_class(string teacher_id, string jwt, string course_id, string class_id, string title, string description, string content_url, OracleConnection connection)//修改教学课程
        {
            Function_result return_result = new Function_result();
            return_result.Code = -99;
            return_result.Message = "Unknown Error";
            ...
            //如果课程的确是当前教师拥有的，那么就查询当前要修改的教学任务编号是否在当前课程中
            check = "SELECT count(*) FROM class WHERE id = '" + course_id + "'\n";
            check = check + "and id = '" + class_id + "'";
            count = 0;
            try
            {
                using (OracleCommand command = new OracleCommand(check, connection))
            ...
            //以下为数据库插入操作
            string action = "update class\n";
            action = action + "set title = '" + title + "',\n";
            action = action + "description = '" + description + "',\n";
            action = action + "content_url = '" + content_url + "'\n";
            action = action + "where course_id = '" + course_id + "'\n";
            action = action + "and id = '" + class_id + "'\n";
            try
            {
                using (OracleCommand command = new OracleCommand(action, connection))
                {
        ...
        public static Function_result Delete_class(string teacher_id, string jwt, string course_id, string class_id, OracleConnection connection)//删除教学内容
        {
            Function_result return_result = new Function_result();
            return_result.Code = -99;
            return_result.Message = "Unknown Error";
            ...
            string action = "delete from class\n";
            action = action + "where course_id = '" + course_id + "'\n";
            action = action + "and id = '" + class_id + "'\n";
            try
            {
        ...
        public static Function_result Get_class_id_by_course(string user_type, string user_id, string jwt, string course_id, OracleConnection connection)//用户查询一门课程包含的教学任务
        {
            Function_result return_result = new Function_result();
            return_result.Code = -99;
            return_result.Message = "Unknown Error";
            ...
            //鉴权通过的话，就先查询教学任务的数量
            string check = "SELECT COUNT(*) FROM class WHERE course_id = '" + course_id + "'";
            int count = 0;
            try
            {
                using (OracleCommand command = new OracleCommand(check, connection))
            ...
            {
                return_result.Code = -13;//-13表示查询教学任务数量的sql操作无法顺利执行
                return_result.Message = "SQL Error";
                return return_result;
            }
            if (count == 0)
            ...
            //若课程存在，获得查询值
            check = "SELECT id FROM class WHERE course_id = '" + course_id + "'";
            string ans = "";
            try
            {
            ...
            {
                return_result.Code = -14;//-14表示查询教学任务ID的sql操作无法顺利执行
                return_result.Message = "SQL Error";
                return return_result;
            }
        ...
        public static Function_result Get_info_by_class_id(string course_id, string class_id, OracleConnection connection)//根据课程ID和教学内容ID获取教学内容信息
        {
            Function_result return_result = new Function_result();
            return_result.Code = -99;
            return_result.Message = "Unknown Error";
            string action = "SELECT title,description,content_url,create_time FROM class WHERE course_id = '" + course_id + "'";
            action = action + "and id = '" + class_id + "'\n";
            string ans = "";
            try
            {
                using (OracleCommand cmd = new OracleCommand(action, connection))
                {
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            ans = ans + reader["content_url"].ToString() + "\t\r";
